            ...
            if((codI != "" && passI != "" && userI != "") || (codA != ""  && passA != ""  && userA != ""))
            {
                switch (Device.RuntimePlatform)
                {
                    case Device.Android:
                        ...
                        result = await ControlloCodice.AggiungiUtente(u);
                        break;
                    case Device.iOS:
                        Utente f = new Utente(userI, passI, codI);
                        result = await ControlloCodice.AggiungiUtente(f);
                        break;
                }
                if (result) await DisplayAlert("Fondomerende", "L'utente è stato aggiunto con successo", "Ok");
                else await DisplayAlert("Fondomerende", "L'utente è gia presente o il codice è gia stato scelto o l'utente non esiste" , "Ok");
            }
            else
            {
